...
history <- bind_rows(history2DF(smaller_history, "small", 20),
                         history2DF(baseline_history,"baseline",20),
                         history2DF(bigger_history, "big",20)
                         ) 
...
ggplot(data=history, mapping=aes(x=epoch)) + 
  geom_point(aes(y=accuracy,col="Entrenamiento")) +
  geom_line(aes(y=accuracy,col="Entrenamiento")) +
  geom_point(aes(y=val_accuracy,col="Prueba")) +
  geom_line(aes(y=val_accuracy,col="Prueba")) +
...
NOTA DE PABLO: Nuestro dataset actual tiene solo 200 datapoints, así que vamos a tener que releer el dataset original y recuperar 500 columnas o datapoints. Luego, tendremos que separarlo en conjuntos de entrenamiento y de testeo. Lo realizaremos de forma tal que el dataset de entrenamiento tenga 400 datapoints y el de testeo 100.
...
baseline_history_20_200_400 <- bind_rows(
  get_history_df(baseline_history_20,  20, 20), 
  get_history_df(baseline_history_200, 20, 200),
  get_history_df(baseline_history_400, 20, 400)
) 
...
ggplot(data=baseline_history_20_200_400, mapping=aes(x=epoch)) + 
  ...
  geom_point(aes(y=val_accuracy,col="Prueba")) +
  geom_line(aes(y=val_accuracy,col="Prueba")) +
  facet_wrap(~dataset_size,ncol=3) + theme(axis.text.x = element_text(angle = 60),legend.position = "bottom") +
   labs(subtitle="Proceso de Entrenamiento de 3 modelos con diferente complejidad",y="error", x="epoch") + scale_color_manual(values = colors)
```

# Ejercicio  14

Recrear el gráfico que muestra la convergencia de las curvas de error de entrenamiento y de testeo a medida que aumenta el tamaño del dataset de entrenamiento. Este gráifco puede observarse en la sección "Determinación de la cantidad de datos necesarios para entrenamiento" del TutorialBiasVarianza.

Para esto, deberá entrenar distintos modelos con cantidades distintas de datos N={5,10,15,20,...., 190,195,200} datapoints. En cada iteración almacene el último valor de la función de perdida tanto la de entrenamiento (loss) como la de validacion (val_loss). 
