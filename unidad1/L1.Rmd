...
Definiremos un conjunto de funcines para las conversiones de unidades. Para esto utilizaremos la librería `measurements`

```{r}
library(measurements)
```

##### Conversor de metros a centrímetros

*Parámetros:* `q`, <string> longitud en pies escrita en el formato "pies'decimal". Por ejemplo, este podría ser "1'2".

*Salida:* <double> longitud en centímetros.
...
*Parámetros:* `q`, <string> masa expresada en kilogramos en el formato "1lbs".

*Salida:* <double> masa expresada en kilogramos.
...
*Parámetros:* `q`, <string> cantidad expresada con un número seguido de una unidad. Ejemplos: "1M" para un millón, "1K" para mil.

*Salida:* <double> cantidad expresada en pesos.
...
refactorMoney <- function(q){
  ...
  if(q %>% str_detect("K")){
    return(
      1000 * q %>% str_replace("K","") %>% as.numeric()) 
  }
  if(q %>% str_detect("M")){
    return(
      1000000 * q %>% str_replace("M","") %>%
      as.numeric()) 
  }
  q %>% as.numeric()
}
...
Analizamos el dataset y detectamos en qué casos se presentan valores nulos. Así, detectamos un conjunto de columnas que no aportan valores a las predicciones, en algunos casos son nulas y que solo ralentizan el procesamiento. Por estos motivos, optamos por eliminarlas. 
Estas son:
...
También detectamos valores nulos en los datos que representan el desempeño de los jugadores en las distintas posiciones en el caso de los arqueros. En estos casos, optaremos por asignar un cero a los valores NA. 
A continuación, se presentan estas columas:
...
Por otra parte, también transformaremos en cero los valores nulos de las columnas asociadas a las habilidades de los jugadores. Estas son:
...
Finalmente, también se detectaron `NA` en las columas Position y Club que podrían ser reelevantes para nuestras predicciones. Por este motivo, en estos casos optaremos por reemplazar los `NA` por el string "UNKNOWN"

#### Obtención del dataset

A partir de los métodos anteriores, creamos un único pipeline para obtener y formatear nuestro dataset.

```{r}
fifa <- read_csv("data/fifa.csv") %>%
  # Eliminar columnas irrelevantes
  select(-all_of(irrelevant_features)) %>%
  mutate(
    # Covertir unidades
    ...
    # Reemplazar NA
    across(
      .cols = c(Position, Club),
      .fns = ~ . %>% replace_na("UNKNOWN")
    ),
    across(
      .cols = all_of(position_features),
      .fns = ~ . %>% replace_na(0)
    ),
    across(
      .cols = all_of(skill_features),
      .fns = ~ . %>% replace_na(0)
    )
  )
  # %>%
  # Eliminar las filas que aún tienen NA
  # na.omit()
  
fifa
```

La siguiente función obtiene las posiciones de los NA del dataset. La ejecutamos para comprobar que todos los valores nulos fueron removidos.
...
Se prepara los datos para las predicciones. Para esto, utilizaremos el método `simplifica_posicion()` para reducir el conjunto de posiciones posibles y acotaremos el conjunto a las columnas de interés.

```{r}
# Valores para identificar a los jugadores
players_ids <- c("DataPoint", "ID", "Name", "Club", "Nationality")

# Columnas utilizadas en la predicción
recipe_columns <- c(
  # Identificadores
  players_ids,
  # Salida
  "Position", "Overall", "Value",
  # Predictores
  skill_features,
  position_features
)
...
fifa <- fifa %>%
  # Filter columns whose Position is unknown
  filter(Position != "UNKNOWN") %>% 
  # Reduccir las posiciones a una de cuatro posibles valores
  simplificaPosicion() %>%
  # Eliminar columnas irrelevantes
  select(all_of(recipe_columns_clasificacion))
...
Luego, dividimos el dataframe en un conjunto de entrenamiento y de testeo.

```{r}
set.seed(42)
data_split <- fifa %>%  simplificaPosicion() %>% initial_split(prop = 3/4)
...
Se crea una receta cuyo output es la columna `Position` a partir del conjunto de entrenamiento. Se asigna el rol `ID` a aquellas columnas que no se desea que se emplee como predictores. No las eliminamos porque el conservar las identificaciones de los jugadores, nos ayuda en el análisis de los datos.


```{r}
# Crear receta
receta_clasificacion <- 
  recipe(formula = Position ~ ., data = train_data) %>% 
  # Crear roles
  update_role(all_of(players_ids), new_role = "ID") %>%
  # Si aún quedaran NA, se los reemplaza por la mediana de la columna
  step_impute_median() %>%
...
dt_model_clasificacion <- decision_tree() %>%
  set_engine("rpart") %>%
...
wf_position_tree <- 
  workflow() %>% 
  add_model(dt_model_clasificacion) %>% 
...
rf_model_clasificacion <- rand_forest() %>%
  set_engine("ranger") %>%
  set_mode("classification")
...
wf_position_forest <- 
  workflow() %>% 
  add_model(rf_model_clasificacion) %>% 
  add_recipe(receta_clasificacion)

# Entrenar el modelo
wf_position_forest_fit <- 
  wf_position_forest %>%
  fit(data = train_data)
...
```{r}
# Crear receta
receta_clasificacion_mejorada <- 
  recipe(formula = Position ~ ., data = train_data) %>% 
  # Filter columns whose Position is unknown
  step_filter(Position != "UNKNOWN") %>% 
  step_filter(SlidingTackle != 0 | Position == "GK") %>% 
  step_select(-Overall, -Value) %>%
  step_mutate(SqrSlidingTackle = SlidingTackle ** 2 ) %>%
  # Crear roles
  update_role(all_of(players_ids), new_role = "ID") %>%
  # Si aún quedaran NA, se los reemplaza por la mediana de la columna
  step_impute_median() %>%
...
# Crear el modelo de árbol de decisión
dt_model_clasificacion_mejorado <- decision_tree() %>%
  set_engine("rpart") %>%
  set_mode("classification")

# Crear un workflow que vincule el modelo y la receta
wf_position_tree_mejorado <- 
  workflow() %>% 
  add_model(dt_model_clasificacion_mejorado) %>% 
  add_recipe(receta_clasificacion_mejorada)

# Entrenar el modelo
wf_position_tree_fit_mejorado <- 
  wf_position_tree_mejorado %>%
  fit(data = train_data)

# Obtener métricas
augment(wf_position_tree_fit_mejorado, test_data) %>%
  metrics(truth = Position, .pred_class)
...
receta_regresion_overall <- 
  # Crear la receta 
  recipe(Overall ~ ., data = train_data) %>%
  # Cambiar roles de columnas no usadas en la prediccion
  update_role(all_of(players_ids), new_role = "ID") %>%
  # Asegurarnos que el valor no se utiliza para la predicción
  update_role(Value, new_role = "not used") %>%
  # Si aun quedaran NA se reemplazan por la mediana
...
# Crear el modelo de árbol de decisión
dt_model_overall <- decision_tree() %>%
  set_engine("rpart") %>%
  set_mode("regression")
...
wf_overall_tree <-
  workflow() %>% 
  add_model(dt_model_overall) %>% 
  add_recipe(receta_regresion_overall)
...
```{r}
overall_tree_aug <- 
  augment(wf_overall_tree_fit, test_data)
...
```{r}
overall_tree_aug %>% 
  metrics(truth = Overall, .pred)
```

NOTAS:
* RMSE significa Root Mean Squared Error o desviación estándar
* RSQ: significa Root Squared o variance
...
```{r}
overall_forest_aug <- 
  augment(overall_forest_fit, test_data)
...
#### Mejora del modelo de árbol de decisión

Se observó que el modelo de árbol de decisión presentaba un error significativamente mayor al del random forest. Esto se debe a que se adoptó un engine equivocado, pues `rpart` se emplea para modelos de clasificación. Como alternativa, utilizamos el engine `partykit`
...
dt_model_overall_improved <- decision_tree(tree_depth = 10) %>%
  set_engine("partykit") %>% 
  set_mode("regression")

# Crear un workflow que vincule el modelo y la receta
wf_overall_tree_improved <- 
  workflow() %>% 
  add_model(dt_model_overall_improved) %>% 
  add_recipe(receta_regresion_overall)

# Entrenar el modelo
wf_overall_tree_fit_improved <- 
  wf_overall_tree_improved %>%
  fit(data = train_data)

# Obtener predicciones y probabilidades
overall_tree_aug_improved <- 
  augment(
    wf_overall_tree_fit_improved,
    test_data)
overall_tree_aug_improved
...
Analizando los datos se observó que rpart definía un conjunto de 12 categorías, es decir, 12 valores posibles de Overall que asignaba a los jugadores según el árbol de decisión. Partysnip, en cambio, asigna un conjunto mayor de valores. 

Por este motivo, partysnip obtiene valores que se asemejan más a los originales. Esto se observa en la reducción del RMSE (de 3.07 a 2.72) y del MAE (de 2.37 a 2.07). Sin embargo, al generar un conjunto mayor de valores, se ve más afectado por los extremos y esto se refleja en un RSQ mayor (de 0.80 a 0.84). 
...
```{r}
receta_regresion_value <- 
  recipe(
    formula = Value ~ .,
    data = train_data %>% filter(Value != 0)
  ) %>%
  # Cambiar roles de columnas no usadas en la prediccion
  update_role(all_of(players_ids), new_role = "ID") %>%
  update_role(Overall, new_role = "not used") %>%
  # Crear indicadores numéricos para predictores de tipo char
  step_dummy(all_nominal_predictors()) %>%
  # Eliminar valores únicos (varianza = 0)
  step_zv(all_predictors()) 
...
wf_value_tree <-
  workflow() %>% 
  add_model(dt_model_value) %>% 
  add_recipe(receta_regresion_value)
...
```{r}
value_tree_aug <- 
  augment(value_tree_fit, test_data)
...
```{r}
value_tree_aug %>% 
  metrics(truth = Value, .pred)
```

Se observa que los valores de las métricas RMSE y MAE son números elevados pues deben compararse con los valores de la columna Value que están en el orden de los millones.
...
```{r}
value_forest_aug <- 
  augment(value_forest_fit, test_data)
...
De la misma manera que en el caso anterior, mejoramos el modelo cambiando el engine `rpart` por `partykit`.
...
# Crear modelo ajustado
dt_model_value_improved <- decision_tree(tree_depth = 40) %>%
  set_engine("partykit") %>% 
  set_mode("regression")

# Crear un workflow que vincule el modelo y la receta
wf_value_tree_improved <- 
  workflow() %>% 
  add_model(dt_model_value_improved) %>% 
  add_recipe(receta_regresion_value)

# Entrenar el modelo
wf_value_tree_fit_improved <- 
  wf_value_tree_improved %>%
  fit(data = train_data)

# Obtener predicciones y probabilidades
value_tree_aug_improved <- 
  augment(
    wf_value_tree_fit_improved,
    test_data)
value_tree_aug_improved
...
Suponiendo que el stakeholder esta interesado en comprar un mediocampista (MID) y que tiene una restricción presupuestaria de 100, debemos recomendar algún jugador dentro del conjunto de testeo.

### Ejercicio 15

Con la idea de comprar un mediocampista confiaremos además en la sugerencia del clasificador, para encontrar de entre todos los jugadores mediocampistas del conjunto de testeo, aquellos que según el clasificador cumplen estas características de mediocampistas.
...
Utilizando el mejor clasificador del ejercicio anterior (según la medida de accuracy), filtrar todos los jugadores del conjunto de testeo (dataTest) que cumplan con la posición de mediocampista (clase MID) y que además el clasificador los haya clasificado como mediocampistas.
Reportar un subset, que contenga las características físicas y mentales (columnas 55 a la 88 del dataset original, `skill_features`), nombre del jugador (Name), posicion real (Position), y posición predicha por el clasificador (Position_Pred).


```{r}
# Obtener una predicción de la posición y añadirla al objeto test_data
# en la columna .pred_class. Se emplea el método árbol de decisión.
position_tree_fifa_aug <- 
  augment(wf_position_tree_fit, test_data)

mids_candidatos <- 
  position_tree_fifa_aug %>%
  filter(.pred_class == "MID" & Position == "MID")  %>%
  select(
    ID, Name, all_of(skill_features), Position, .pred_class,
  ) %>% 
  rename_at(".pred_class", ~"Position_pred")
...
```{r}
# Obtener los resultados de la mejor predicción sobre el objeto fifa y
# añadirlos como columnas nuevas
overall_forest_fifa_aug <- 
  augment(overall_forest_fit, fifa) 


fifa_predictions <- 
  overall_forest_fifa_aug %>% 
  rename(Overall_Pred = .pred) %>%
  mutate(Overall_Err = Overall - Overall_Pred)
...
value_forest_fifa_prediction <-
  # Obtener dataset con predicciones sobre el objeto fifa
  augment(value_forest_fit, fifa) %>%
...
fifa_predictions <-
  inner_join(
    fifa_predictions,
    value_forest_fifa_prediction %>%
        # Se eliminan columnas para evitar duplicados 
        select(ID, Value_Pred, Value_Err),
    join_by(ID == ID)
  )
...
Reordenar los mediocampistas según la variable de puntaje (Overall_Pred) que predijo el regresor en forma descendente y mostrar el top 10 en un gráfico que muestre {Nombre de jugador, Overall, Overall_Pred, Overall_Err}
...
ggplot(
  ...
  data = fifa_predictions %>% 
    # Sort values
    arrange(desc(Overall_Pred)) %>%
    # Keep only the top ten
    head(10) %>% 
    # Select only some columns
    select(Name, Overall, Overall_Pred, Overall_Err) 
) +
  geom_bar(
    mapping = aes(x = Name, y = Overall),
    stat="identity",
    fill = "#94FFD8",
    alpha = 0.5,
    color = "#4bffbd") + 
  geom_bar(
    mapping = aes(x = Name, y = Overall_Pred),
    stat="identity",
    fill = "#A3D8FF",
    alpha = 0.5,
    color = "#58b9ff") +
  labs(title="Overall vs Overall Prediction", x="Nombre de jugador", y= "Overall") +
...
mids_candidatos <- mids_candidatos %>% 
  inner_join(
    fifa_predictions %>% select(
      ID,
      Value, Value_Pred, Value_Err,
      Overall, Overall_Pred, Overall_Err),
    join_by(ID == ID)
  )

# Create bar graph
ggplot(
  data =  mids_candidatos %>% 
    # Sort values
    arrange(desc(Value_Pred)) %>%
    # Keep only the top ten
    head(10) %>% 
    # Select only some columns
    select(Name, Value, Value_Pred, Value_Err) 
) +
  geom_bar(
    ...
    color = "#58b9ff") + 
  geom_bar(
    mapping = aes(x = Name, y = Value_Pred),
    stat="identity",
    fill = "#ff76ce",
    alpha = 0.4,
    color = "#ff32b7") +
  labs(title="Value vs Value Prediction", x="Nombre de jugador", y= "Value") +
  theme(axis.text.x = element_text(angle = 45, hjust=1))
```
...
¿Cuales de esos 5 jugadores hubiesen resultado en buenas decisiones, cuáles no y por qué?

Para hallar la respuesta, en primer lugar, obtendremos los mejores jugadores a partir de los datos reales

```{r}
# Mejores jugadores según datos reales
fifa %>%
  filter(Position == "MID") %>%
  filter(Value != 0 & Value <= 500000) %>%
  arrange(desc(Overall)) %>%
  head(20) %>%
  select(Name, Overall, Value)
```
Se observa que ninguno de la predicción se encuentra entre los mejores 20 jugadores. Esto se debe a que los valores extremos de Value generan muchos errores en la predicción de este valor. 
...
Para responder esto, obtenemos los mejores mediocampistas.
```{r}
fifa %>%
  filter(Position == "MID") %>%
  filter(Value != 0 & Value <= 500000) %>%
  ...
  select(Name, Overall, Value)
```
RTA: Se observa que un buen número de jugadores que serían buenas opciones para las restricciones fueron descartados por una predicción de la clasificación errónea.
...
ggplot() +
  geom_jitter(
    data = fifa_predictions %>% filter(Value_Pred <= 500000 & Value < 800000),
    mapping = aes(x = Overall, y = Value),
    color= "#A3D8FF",
    alpha = 0.25) + 
  geom_jitter(
    data = fifa_predictions %>% filter(Value_Pred > 500000 & Value < 800000),
    ...
    color= "#6C6EA0",
    alpha = 0.25) +
  labs(title = "Eficacia de la predicción")
...
Reportar el top 10 de la medida de rendimiento {Nombre de Jugador, medida_rendimiento, Overall_Pred, Overall, Value_Pred, Value, Overall_Err, Value_Err}

```{r}
fifa_predictions %>% 
  filter(Value != 0) %>%
  mutate(Score = (Overall / log(Value) ) * 100000000) %>%
  arrange(desc(Score)) %>%
  head(10) %>%
  select(
    Name, Score, Overall, Overall_Pred, Overall_Err, Value, Value_Pred, Value_Err
  )
```
